...
export default function OptionsModal() {
    ...
    return (
        <div className={`relative ${showOptions ? 'z-[2] md:z-10': ''}`}>
            <button onClick={showOptions ? close : open} className={`${showOptions ? 'hover:bg-red-100 hover:text-red-500 hover:border-red-200 z-10' : ''} relative w-[50px] aspect-square bg-white rounded-full flex items-center justify-center border border-gray-300`}>
                <span className={`${showOptions ? 'rotate-[90deg]' : ''} transition-transform duration-300`}>
                    {showOptions ? <MdClose size={20} /> : <MdMoreHoriz size={20} />}
                </span>
            </button>

            <div onClick={handleClick} className={`${showOptions ? '' : 'opacity-0 pointer-events-none'} transition-opacity duration-300 absolute top-0 right-[calc(100%_+_20px)] w-max min-w-[300px] border border-gray-300`}>
